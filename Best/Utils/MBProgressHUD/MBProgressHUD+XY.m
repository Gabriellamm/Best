...
{
    UIView  *view = isWindow? (UIView*)[UIApplication sharedApplication].delegate.window:[(AppDelegate *)KAppDelegate getCurrentUIVC].view;
...
+ (void)hideHUD
{
    ...
    [self hideAllHUDsForView:[(AppDelegate *)KAppDelegate getCurrentUIVC].view animated:YES];
}
...
+ (void)showTopTipMessage:(NSString *)msg isWindow:(BOOL) isWindow{
    ...
    if (isWindow) {
        ...
        [[(AppDelegate *)KAppDelegate getCurrentUIVC].view addSubview:label];
