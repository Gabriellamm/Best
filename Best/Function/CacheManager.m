...
+ (NSString *)getCacheSize {
    ...
    NSString *totleSize = [CacheManager getCacheSizeWithFilePath:cachesPath];
    return totleSize;
}
...
+ (BOOL)clearCache {
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
    BOOL isAlreadyClearCache = [CacheManager clearCacheWithFilePath:cachesPath];
    return isAlreadyClearCache;
}
+ (void)readyClearCache {
    BOOL isClearCache = [CacheManager clearCache];
    if (isClearCache) {
        DLog(@"清理完毕");
    } else {
        DLog(@"清理失败");
